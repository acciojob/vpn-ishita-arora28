...
public class ConnectionServiceImpl implements ConnectionService {
        ...
        if(user.getConnected()){
            ...
            for(ServiceProvider sp:serviceProviderList){
                ...
                for(Country country:countryList){
                    if(country.getCountryName().toString().equals(countryName) && smallest_id>sp.getId()){
                        smallest_id=sp.getId();
                        serviceP=sp;
                        c=country;
                    }
                }
            ...
            if(serviceP!=null){
                ...
                newConn.setServiceProvider(serviceP);

                String maskedIp=c.getCode()+"."+serviceP.getId()+"."+user.getId();
                user.setMaskedIp(maskedIp);
                user.setConnected(true);
                user.getConnectionList().add(newConn);

                userRepository2.save(user);
                serviceProviderRepository2.save(serviceP);
            }
        }
        return user;
        }
    ...
    public User communicate(int senderId, int receiverId) throws Exception {
        ...
        if(receiver.getMaskedIp() != null){
            ...
            countryName = receiver.getCountry().getCountryName().toString();
